                        ...
                        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
                            <div>
                                <div className="inline-flex items-center gap-2 rounded-full glass px-4 py-2 text-vanilla-50 border border-vanilla-100/15">
                                    ...
                                    <span className="text-sm font-semibold italic">{locale === 'en' ? 'The final step before indulgence.' : 'Dernière étape avant le goût.'}</span>
                                </div>
                                ...
                                <p className="mt-3 text-lg text-vanilla-100/75 max-w-2xl">
                                    {locale === 'en'
                                        ? <>Adjust quantities and complete your order. <span className="text-vanilla-50 font-semibold italic underline decoration-gold-500 underline-offset-4">Tip</span>: a gift tube paired with a longer pod delivers an immediate premium feel.</>
                            ...
                            {!isEmpty && (
                                <div className="rounded-3xl glass p-6 w-full lg:w-[420px] text-vanilla-50 border border-vanilla-100/15">
                                    <div className="flex items-center justify-between gap-3">
                                        <p className="text-sm font-semibold">{locale === 'en' ? 'Free shipping goal' : 'Objectif livraison offerte'}</p>
                                        <p className="text-xs text-vanilla-100/70">
                                            {subtotal >= FREE_SHIPPING_THRESHOLD ? (locale === 'en' ? 'Unlocked!' : 'Seuil atteint !') : locale === 'en' ? `${fmt.format(missingForFree)} to go` : `Encore ${fmt.format(missingForFree)}`}
                                        </p>
                                    </div>
                                    <div className="mt-3 h-2 rounded-full bg-vanilla-50/10 overflow-hidden">
                                        ...
                                        {locale === 'en' ? 'Marketing note: the free shipping threshold remains configurable.' : 'Paramètre marketing : seuil “livraison offerte” configurable.'}
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="grid lg:grid-cols-12 gap-12 items-start">
                        ...
                        <section className="lg:col-span-8">
                            <div className="flex items-center justify-between gap-4">
                                <h2 className="font-display text-3xl italic">{locale === 'en' ? 'Items' : 'Articles'}</h2>
                                {!isEmpty && (
                                    <button
                                        onClick={clearCart}
                            ...
                            {isEmpty ? (
                                <div className="mt-8 rounded-[32px] bg-white border border-cacao-900/5 p-10 text-center">
                                    ...
                                    <h3 className="font-display text-2xl italic">{locale === 'en' ? 'Your cart is empty' : 'Votre panier est vide'}</h3>
                                    <p className="mt-3 text-cacao-600">{locale === 'en' ? 'Choose a length, a grade and leave with a truly premium vanilla.' : 'Choisissez une longueur, un grade, et repartez avec une vanille premium.'}</p>
                                    <Link
                                ...
                                <div className="mt-8 space-y-6">
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                                                <div className="flex items-center gap-6 flex-1 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="mt-1 text-sm font-bold tracking-tight text-gold-600">
                                                            {fmt.format(item.price / 100)} <span className="text-[10px] uppercase font-bold text-cacao-400">{locale === 'en' ? '/ unit' : '/ unité'}</span>
                                                        </p>
                                                        {item.variant?.title ? (
                                                            <p className="mt-2 text-xs font-semibold uppercase tracking-widest text-jungle-500">
                                                                {locale === 'en' ? 'Selected variant:' : 'Variante choisie :'} {item.variant.title}
                                                            </p>
                                                        ) : null}
                                                        ...
                                                        <div className="mt-3 flex flex-wrap gap-2">
                                                            <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 bg-vanilla-100 border border-cacao-900/5 text-[10px] font-bold uppercase tracking-widest text-jungle-800 shadow-none">
                                                                ...
                                                                {locale === 'en' ? 'Premium selection' : 'Sélection premium'}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center justify-between sm:justify-end gap-10 border-t sm:border-t-0 border-cacao-900/5 pt-5 sm:pt-0">
                                                    ...
                                                    <div className="text-right min-w-[100px]">
                                                        <p className="text-[10px] font-bold uppercase tracking-widest text-cacao-400">{locale === 'en' ? 'Total' : 'Total'}</p>
                                    ...
                                    <div className="mt-12 rounded-[32px] bg-gradient-to-br from-white to-vanilla-50 border border-cacao-900/5 p-8 relative overflow-hidden shadow-none">
                                        <div className="relative z-10">
                                            <p className="text-sm font-bold uppercase tracking-widest text-gold-600">{locale === 'en' ? 'Did you know?' : 'Le saviez-vous ?'}</p>
                                            <div className="mt-4 grid sm:grid-cols-2 gap-8 text-sm">
                                                <div className="flex gap-4">
                                                    ...
                                                    <p className="text-cacao-700 leading-relaxed font-medium italic">
                                                        {locale === 'en' ? 'Discover our premium gift tubes for elegant presentation and optimal preservation.' : 'Découvrez nos tubes cadeaux premium pour une conservation optimale et un style unique.'}
                                                    </p>
                                                </div>
                                                <div className="flex gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-gold-100 border border-gold-200 flex items-center justify-center shrink-0">
                                                    ...
                                                    <p className="text-cacao-700 leading-relaxed font-medium italic">
                                                        {locale === 'en' ? 'Zero waste tip: an exhausted pod can still perfume a jar of sugar for months.' : 'Anti-gaspi : une gousse épuisée peut encore parfumer votre bocal de sucre pendant des mois.'}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <aside className="lg:col-span-4">
                            <div className="lg:sticky lg:top-24 space-y-6">
                                <div className="rounded-[40px] bg-jungle-900 p-8 text-vanilla-50 relative overflow-hidden shadow-none border border-vanilla-100/10">
                                    ...
                                    <div className="relative z-10">
                                        <div className="flex items-center justify-between">
                                            <h3 className="font-display text-2xl italic">{locale === 'en' ? 'Summary' : 'Récapitulatif'}</h3>
                                        ...
                                        <div className="mt-10 space-y-4 text-sm font-medium">
                                            <div className="flex items-center justify-between border-b border-vanilla-100/5 pb-4">
                                                <p className="text-vanilla-100/60 uppercase tracking-widest text-[10px] font-bold">{locale === 'en' ? 'Products' : 'Produits'}</p>
                                                <p className="font-bold">{fmt.format(subtotal)}</p>
                                            </div>
                                            <div className="flex items-center justify-between border-b border-vanilla-100/5 pb-4">
                                                <p className="text-vanilla-100/60 uppercase tracking-widest text-[10px] font-bold">{locale === 'en' ? 'Shipping' : 'Livraison'}</p>
                                                <p className="italic text-vanilla-100/50">{locale === 'en' ? 'Calculated next' : "A l'étape suivante"}</p>
                                            </div>
                                            <div className="pt-4 flex items-center justify-between">
                                                <p className="text-xl italic font-display">{locale === 'en' ? 'Estimated total' : 'Total estimé'}</p>
                                        ...
                                        >
                                            {locale === 'en' ? 'Proceed to checkout' : 'Passer au paiement'}
                                        ...
                                        <div className="mt-6 grid gap-3">
                                            {[
                                                locale === 'en' ? '100% secure payment and instant confirmation' : 'Paiement 100% sécurisé et confirmation immédiate',
                                                locale === 'en' ? 'Shipping calculated before final validation' : 'Livraison calculée avant validation de la commande',
                                                locale === 'en' ? 'Premium vanilla shipped from controlled stock' : 'Vanille premium expédiée depuis un stock contrôlé'
                                            ].map((message) => (
                                ...
                                <div className="rounded-[32px] bg-vanilla-100/50 border border-cacao-900/5 p-6 shadow-none">
                                    <h4 className="text-xs font-bold uppercase tracking-widest text-jungle-900">{locale === 'en' ? 'Quality commitment' : 'Engagement Qualité'}</h4>
